...
static void *               cmdline_function_data_list[CMDLINE_MAX_COMMANDS];
...
static void * cmdline_exec_data = NULL;
...
void cmdlineAddCommand(u08* newCmdString, CmdlineFuncPtrType newCmdFuncPtr, void * data)
{
	// add command string to end of command list
	strcpy((char *) CmdlineCommandList[CmdlineNumCommands], (char *) newCmdString);
	// add command function ptr to end of function list
	CmdlineFunctionList[CmdlineNumCommands] = newCmdFuncPtr;
        cmdline_function_data_list[CmdlineNumCommands] = data;
...
void cmdlineProcessInputString(void)
{
    ...
    for (cmdIndex = 0; cmdIndex < CmdlineNumCommands; cmdIndex++) {
        len = strlen(CmdlineCommandList[cmdIndex]);
        if (!strncmp(CmdlineCommandList[cmdIndex], (char *) CmdlineBuffer, len)) {
            // user-entered command matched a command in the list (database)
            // run the corresponding function
            CmdlineExecFunction = CmdlineFunctionList[cmdIndex];
            cmdline_exec_data = cmdline_function_data_list[cmdIndex];
...
void cmdlineMainLoop(void)
{
	...
	{
		// run it
		CmdlineExecFunction(cmdline_exec_data);
		// reset
		CmdlineExecFunction = 0;
                cmdline_exec_data   = NULL;
		// output new prompt
		cmdlinePrintPrompt();
	}
}
